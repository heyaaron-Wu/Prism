...
项目的核心原则很简单：AI 可以帮助整理市场信号、验证结果、风险上下文和系统状态，但真实交易决策必须保持可解释、可审计，并由人类手动控制。

## 当前状态

Prism 当前处于 **Phase 3c Shadow 验证期**。

- 当前验证进度：`0/5` clean days
- 实盘模式：human-in-the-loop / 人工手动执行
- Validator v2：仅 shadow compare
- Event Intelligence：仅 display-only
- Decision Cards：解释与可观测性层
- 生产交易链：不被 display-only 模块修改
...
本仓库当前用于 Prism 的公开项目概览，记录系统架构、验证流程、安全边界、当前进度和路线图。
...
英文和中文文档应同步修订。当项目状态、安全边界、验证状态或路线图发生变化时，两种语言版本应在同一次维护中一起更新。
